...
class  UsersController < BaseController
  ...
  def show
    ...
    @microposts = Micropost.where(:user_id=>params[:id]).order("created_at DESC").paginate(:page => params[:page], :per_page => 10)
  end
	def new
		@user = User.new
	end
  ...
  def following
    ...
    @microposts = Micropost.where(:user_id=>@users.collect(&:id)).order("created_at DESC").paginate(:page => params[:page], :per_page => 10)
    render 'show_follow'
  end
  def followers
    @title = "粉丝"
